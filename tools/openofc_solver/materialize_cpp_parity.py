...
def main() -> None:
    ...
    if correct_policy in policy_text:
        print("COFCBaselinePolicy.cpp: production card mapping already materialized")
    else:
        replace_exact(
            POLICY,
    ...
    selftest_text = SELFTEST.read_text(encoding="utf-8-sig")
    correct_card = "int Card(int rank, int suit) {\n  return suit * 13 + (rank - 2);\n}\n"
    if correct_card in selftest_text:
        print("COFCBaselinePolicySelftest.cpp: production card mapping already materialized")
    else:
        replace_exact(
            SELFTEST,
