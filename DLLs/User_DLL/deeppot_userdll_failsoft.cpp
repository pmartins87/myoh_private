//******************************************************************************
// DeepPot OpenHoldem user.dll adapter — fail-soft recovery v2
...
struct HandMemory {
  ...
  HandMemory()
      : snapshots(), have_cards(false), flop(), hole(), have_anchor(false), anchor(),
        pending_handreset_observations(0), last_betround(-1), last_common_cards(-1) {}
...
int IntSymbol(const char* name) { return static_cast<int>(GetSymbol(name)); }
...
bool ValidChair(int chair, int nchairs) { return chair >= 0 && chair < nchairs; }
...
bool EnsureStrategyLoaded(std::string* error) {
  ...
  WriteLog("[DeepPot] runtime loaded adapter=%s root=%s\n",
           const_cast<char*>(kAdapterVersion), const_cast<char*>(root.c_str()));
...
int CardCode(const deeppot_runtime::Card& card) { return (card.rank - 2) * 4 + card.suit; }

bool CardsValidAndUnique(const std::array<deeppot_runtime::Card, 3>& flop,
...
bool SameCardIdentity(const std::array<deeppot_runtime::Card, 3>& flop_a,
...
bool ReadCurrentCards(std::array<deeppot_runtime::Card, 3>* flop,
                      std::array<deeppot_runtime::Card, 2>* hole) {
  if (!ReadCard("$$pr0", "$$ps0", &(*hole)[0]) ||
      !ReadCard("$$pr1", "$$ps1", &(*hole)[1]) ||
      !ReadCard("$$cr0", "$$cs0", &(*flop)[0]) ||
      !ReadCard("$$cr1", "$$cs1", &(*flop)[1]) ||
      !ReadCard("$$cr2", "$$cs2", &(*flop)[2])) return false;
...
bool CoherentAnchorSnapshot(const deeppot_runtime::LiveScrapeSnapshot& s) {
  if (s.nchairs < 2 || s.nchairs > 10 || !ValidChair(s.userchair, s.nchairs) ||
      !ValidChair(s.dealerchair, s.nchairs)) return false;
...
void ResetForNewHand(const char* reason) {
  if (g_hand.have_cards || g_hand.have_anchor || !g_hand.snapshots.empty()) {
    WriteLog("[DeepPot] HAND_BOUNDARY reason=%s previous_snapshots=%d\n",
             const_cast<char*>(reason), static_cast<int>(g_hand.snapshots.size()));
  }
...
void SetHandAnchorIfMissing(const deeppot_runtime::LiveScrapeSnapshot& raw) {
  ...
  WriteLog("[DeepPot] HAND_ANCHOR nchairs=%d dealer=%d hero=%d dealt=0x%X N=%d\n",
           g_hand.anchor.nchairs, g_hand.anchor.dealerchair, g_hand.anchor.userchair,
           g_hand.anchor.playersdealtbits, g_hand.anchor.nplayersdealt);
}

bool SameSnapshot(const deeppot_runtime::LiveScrapeSnapshot& a,
                  const deeppot_runtime::LiveScrapeSnapshot& b) {
  return a.nchairs == b.nchairs && a.dealerchair == b.dealerchair &&
         a.userchair == b.userchair && a.playersdealtbits == b.playersdealtbits &&
         a.playersplayingbits == b.playersplayingbits && a.foldbits2 == b.foldbits2 &&
...
void RememberSnapshot(const deeppot_runtime::LiveScrapeSnapshot& s) {
  ...
  if (g_hand.snapshots.size() > kMaxHandSnapshots) g_hand.snapshots.erase(g_hand.snapshots.begin());
}

void ApplyHandAnchor(deeppot_runtime::LiveScrapeSnapshot* current,
                     std::vector<std::string>* reasons) {
  if (!g_hand.have_anchor) return;
  const deeppot_runtime::LiveScrapeSnapshot& a = g_hand.anchor;
  if (current->nchairs != a.nchairs) { current->nchairs = a.nchairs; if (reasons) reasons->push_back("nchairs_from_hand_anchor"); }
  if (current->userchair != a.userchair) { current->userchair = a.userchair; if (reasons) reasons->push_back("userchair_from_hand_anchor"); }
  if (current->dealerchair != a.dealerchair) { current->dealerchair = a.dealerchair; if (reasons) reasons->push_back("dealer_from_hand_anchor"); }
  ...
  if (current->nplayersdealt != anchored_n && reasons) reasons->push_back("nplayersdealt_from_hand_anchor");
...
void ObserveLifecycle() {
  ...
  const bool reset_callback_confirmed = g_hand.pending_handreset_observations > 0 &&
...
void CaptureLiveObservation() {
    ...
    if (ReadCurrentCards(&flop, &hole)) {
      if (g_hand.have_cards && !SameCardIdentity(g_hand.flop, g_hand.hole, flop, hole)) {
        if (g_hand.pending_handreset_observations > 0 || CoherentAnchorSnapshot(raw)) {
          ResetForNewHand(g_hand.pending_handreset_observations > 0
                              ? "handreset_confirmed_card_identity_changed"
                              : "card_identity_changed_with_coherent_public_state");
        }
      }
      if (!g_hand.have_cards || SameCardIdentity(g_hand.flop, g_hand.hole, flop, hole)) {
        g_hand.flop = flop; g_hand.hole = hole; g_hand.have_cards = true;
      }
    }
  }
  ...
  if (g_hand.pending_handreset_observations > 0) --g_hand.pending_handreset_observations;
}

bool GetDecisionCards(std::array<deeppot_runtime::Card, 3>* flop,
                      std::array<deeppot_runtime::Card, 2>* hole,
                      bool* from_cache) {
  ...
  if (ReadCurrentCards(&live_flop, &live_hole)) {
    if (!g_hand.have_cards || SameCardIdentity(g_hand.flop, g_hand.hole, live_flop, live_hole)) {
      g_hand.flop = live_flop; g_hand.hole = live_hole; g_hand.have_cards = true;
      *flop = live_flop; *hole = live_hole; if (from_cache) *from_cache = false; return true;
    }
  ...
  if (g_hand.have_cards) {
    *flop = g_hand.flop; *hole = g_hand.hole; if (from_cache) *from_cache = true; return true;
  }
  return false;
}

bool NormalizeSnapshotForRecovery(deeppot_runtime::LiveScrapeSnapshot* current,
                                  std::vector<std::string>* reasons) {
  ApplyHandAnchor(current, reasons);
  if (current->nchairs < 2 || current->nchairs > 10) {
    bool restored = false;
    for (std::vector<deeppot_runtime::LiveScrapeSnapshot>::const_reverse_iterator it = g_hand.snapshots.rbegin();
         it != g_hand.snapshots.rend(); ++it) {
      if (it->nchairs >= 2 && it->nchairs <= 10) {
        current->nchairs = it->nchairs; restored = true;
        if (reasons) reasons->push_back("nchairs_from_same_hand_history"); break;
      }
    }
    if (!restored) { current->nchairs = 8; if (reasons) reasons->push_back("nchairs_default_8"); }
  }
  if (!ValidChair(current->userchair, current->nchairs)) {
    for (std::vector<deeppot_runtime::LiveScrapeSnapshot>::const_reverse_iterator it = g_hand.snapshots.rbegin();
         it != g_hand.snapshots.rend(); ++it) {
      if (it->nchairs == current->nchairs && ValidChair(it->userchair, current->nchairs)) {
        current->userchair = it->userchair; if (reasons) reasons->push_back("userchair_from_same_hand_history"); break;
      }
    }
  }
  if (!ValidChair(current->dealerchair, current->nchairs)) {
    for (std::vector<deeppot_runtime::LiveScrapeSnapshot>::const_reverse_iterator it = g_hand.snapshots.rbegin();
      ...
      if (it->nchairs == current->nchairs && ValidChair(it->dealerchair, current->nchairs)) {
        current->dealerchair = it->dealerchair; if (reasons) reasons->push_back("dealer_from_same_hand_history"); break;
      }
    }
  }
...
bool BuildPrimaryPublicState(const deeppot_runtime::LiveScrapeSnapshot& s,
                             int* num_players, int* actor_index,
                             std::uint32_t* prior_stay_mask,
                             std::uint32_t* inferred_fold_mask,
                             std::string* error) {
  if (s.nchairs < 2 || s.nchairs > 10 || !ValidChair(s.dealerchair, s.nchairs) || !ValidChair(s.userchair, s.nchairs)) {
    if (error) *error = "invalid chair/dealer scrape"; return false;
  }
  ...
  const int n = BitCount(dealt);
  if (n < 2 || n > 8 || n != s.nplayersdealt) { if (error) *error = "players-dealt count mismatch"; return false; }
  if ((dealt & (static_cast<std::uint32_t>(1) << s.dealerchair)) == 0 ||
      (dealt & (static_cast<std::uint32_t>(1) << s.userchair)) == 0) {
    if (error) *error = "dealer or hero absent from dealt mask"; return false;
  }
  if ((playing & (static_cast<std::uint32_t>(1) << s.userchair)) == 0 ||
      (folded & (static_cast<std::uint32_t>(1) << s.userchair)) != 0) {
    if (error) *error = "hero playing/fold state inconsistent"; return false;
  }
  std::vector<int> order; order.reserve(static_cast<std::size_t>(n));
  ...
  if (static_cast<int>(order.size()) != n || order.back() != s.dealerchair) {
    if (error) *error = "cannot reconstruct fixed Pot Fold action order"; return false;
  }
  int actor = -1;
  for (int i = 0; i < n; ++i) if (order[static_cast<std::size_t>(i)] == s.userchair) { actor = i; break; }
  if (actor < 0) { if (error) *error = "hero absent from action order"; return false; }

  std::uint32_t stay_mask = 0, inferred = 0;
  for (int i = 0; i < actor; ++i) {
    ...
    if (is_folded && is_playing) { if (error) *error = "prior actor simultaneously playing and folded"; return false; }
    if (is_playing) stay_mask |= static_cast<std::uint32_t>(1) << i;
    else if (!is_folded) inferred |= static_cast<std::uint32_t>(1) << i;
  }
  if (deeppot_runtime::Strategy::ScenarioDenseId(n, actor, stay_mask) < 0) {
    if (error) *error = "primary public state is terminal/invalid"; return false;
  }
  *num_players = n; *actor_index = actor; *prior_stay_mask = stay_mask;
...
std::string JoinReasons(const std::vector<std::string>& reasons) {
  ...
  for (std::size_t i = 0; i < reasons.size(); ++i) { if (i != 0) out << ","; out << reasons[i]; }
  return out.str();
}
...
int DeepPotAction() {
  ...
  if (normalized) {
    int n = 0, actor = -1;
    std::uint32_t stay_mask = 0, inferred_fold_mask = 0;
    std::string primary_error;
    if (BuildPrimaryPublicState(current, &n, &actor, &stay_mask, &inferred_fold_mask, &primary_error)) {
      ...
      if (result.ok) {
        ...
        if (inferred_fold_mask == 0 && normalization_reasons.empty() && !cards_from_cache) {
          WriteLog("[DeepPot] HIT EXACT N=%d actor=%d scenario=%d code=%d flop=%d hole=%d action=%s\n",
          ...
          if (inferred_fold_mask != 0) {
            std::ostringstream inferred; inferred << "infer_missing_foldbits_actor_mask=0x" << std::hex << inferred_fold_mask;
          ...
          WriteLog("[DeepPot] HIT RECOVERED_PRIMARY reason=%s N=%d actor=%d scenario=%d code=%d flop=%d hole=%d action=%s\n",
  ...
  } else error = "userchair unavailable with no same-hand recovery";
  ...
  if (normalized) {
    ...
    for (std::size_t i = 0; i < candidates.size(); ++i) {
      ...
      deeppot_runtime::QueryResult q = g_strategy.Query(candidate.num_players, candidate.actor_index,
                                                        candidate.prior_stay_mask, flop, hole);
      if (!q.ok) continue;
      valid_candidates.push_back(candidate); stay_by_key[CandidateKey(candidate)] = q.stay;
    }
    if (!valid_candidates.empty()) {
      const deeppot_runtime::RecoveryConsensus consensus = deeppot_runtime::WeightedPolicyConsensus(
          valid_candidates,
      ...
      if (consensus.resolved) { chosen_stay = consensus.stay; resolution = "CONSENSUS"; }
      else { chosen_stay = stay_by_key.find(CandidateKey(valid_candidates.front()))->second; resolution = "NEAREST_TIEBREAK"; }
      const deeppot_runtime::PublicStateCandidate* chosen = &valid_candidates.front();
      for (std::size_t i = 0; i < valid_candidates.size(); ++i) {
        if (stay_by_key.find(CandidateKey(valid_candidates[i]))->second == chosen_stay) { chosen = &valid_candidates[i]; break; }
      }
      ...
      WriteLog("[DeepPot] HIT RECOVERED_%s candidates=%d min_cost=%d stay_weight=%.3f fold_weight=%.3f chosen_cost=%d reason=%s N=%d actor=%d scenario=%d code=%d action=%s primary_error=%s\n",
               const_cast<char*>(resolution), consensus.candidate_count, consensus.min_cost,
  ...
  if (deeppot_runtime::IsTopPairOrBetter(flop, hole)) {
    WriteLog("[DeepPot] EMERGENCY TP_PLUS -> STAY code=%d reason=public_state_unresolved primary_error=%s\n",
             kEmergencyStayCode, const_cast<char*>(error.c_str()));
    return kEmergencyStayCode;
  }
  WriteLog("[DeepPot] MISS UNRECOVERABLE reason=public_state_unresolved_non_TP_plus primary_error=%s\n",
           const_cast<char*>(error.c_str()));
  return 0;
}
...
DLL_IMPLEMENTS double __stdcall ProcessQuery(const char* pquery) {
  ...
  if (std::strcmp(pquery, "dll$deeppot_action") == 0) return static_cast<double>(DeepPotAction());
  return 0.0;
}
